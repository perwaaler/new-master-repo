...
rudeness_driverA = 0.23;
line_y = imag(B0) + rudeness_driverA*stepsize(1)/stepsize(2)*(real(A0) - real(B0));
decision_dist = imag(A0) - line_y;
% if decision_dist 
rotation_sign = sign(imag(A0) - line_y);
...
if rotation_sign < 0 && decision_state == 0
    ...
    mod_step = [1 0]*stepsize(1)*min(0.12, 0.15*exp(-0.9*(t_min-5))*exp(-5*(d_min-2.8)) );
else
    mod_thetaA = min(pi/30, 0.01*exp(-0.3*(t_min-5))*exp(-2.5*(d_min-2)) );
    mod_thetaB = min(pi/30, 0.01*exp(-0.3*(t_min-5))*exp(-2.5*(d_min-2)) );
    mod_theta = [mod_thetaA mod_thetaB];
    mod_step =  [0,stepsize(2)*min(0.09, 0.2*exp(-0.3*(t_min-7))*exp(-2.5*(d_min-4)) )];
    decision_state = 1;
end
...
stepsize = max(0.05*min_stepsize, stepsize - mod_step);
