...
detection_status_A = 0;
detection_status_B = 0;
...
%       dp = p_engage_EA(A0,B0,stepsize,theta, detec_par);
        if detection_status_A==0 || detection_status_B==0
            ...
            detection_status_A = max(double(rand(1)<dp*alertness(1)),detection_status_A);
            detection_status_B = max(double(rand(1)<dp*alertness(2)),detection_status_B);
            enc_status(end+1) = max(reactA,reactB);
            min_ttcvec(end+1) = t_min;
            min_distvec(end+1) = d_min;
            detection_status = max(detection_status_A,detection_status_B);
            distvec(end+1) = D;
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if  detection_status==1 && real(A0) < real(B0) && imag(A0)<imag(B0)+0.5
            ...
            % take next step
            if detection_status_A==1 && detection_status_B==1
            ...
            elseif detection_status_A==1 && detection_status_B==0
            ...
            elseif detection_status_A==0 && detection_status_B==1
            ...
            plot_pos(A1, B1, pause_length, xinit, r, plot_enc,detection_status_A, detection_status_B) 
