...
plot_sim_walks = 0;                   
plot_setting = 0;
disable_crash = 0;                       % set to 1 disable collision and EA mode, and record free movement patterns
...
% variance of the step-size and step-anlge
var_step0 = 0.001;
...
dist_min_NEA = inf*ones(1,N);           % saves mim sep. dist. for encounters of type NEA
dist_min = inf*ones(1,N);               % saves mim sep. dist. for all encounters
ttc_min = inf*ones(1,N);                % saves mim TTC for all encounters

enc_type = zeros(1,N);                  % vector containing type of each encounter
min_stepsize = 0.01;                    % minimum stepsize
stability_fac = 0.06;                   % helps make course horizontal again after evasive manouver
...
tolerance = 0.3*[0.2, 0.2*20, 0.1*20];
...
d1_theta = 4.0*gamrnd(1^2/0.04, 0.04/1);
p1_theta = 1.0*gamrnd(1^2/0.03, 0.03/1);
d1_step = pi/4*0.6*gamrnd(1^2/0.01, 0.01/1);                              % angle for which maximum deccelaration occurs
p1_step = 15*gamrnd(1^2/0.01, 0.01/1);
amp1_step = 0.06*gamrnd(1^2/0.02, 0.02/1);

theta_mod_par = [-d1_theta, -p1_theta, pi/2];    % parameters of s-function that computes exp_theta(A0)
...
%     while  imag(A0)<4 || real(B0)>-xinit 
      while real(A0) < xinit   && imag(A0)<4 && real(B0) > -xinit && imag(A0)<imag(B0)+1.5 && real(A0)<real(B0)+1.5
        counter = counter + 1;
        nn = nn + 1;
        D = norm(A0-B0);
...
%         dp = p_engage_EA(A0,B0,stepsize,theta, detec_par);
        dp = p_engage_EA_v2(A0,B0,stepsize0,theta, driver_prop, detec_par,farsight);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if  (detection_status==1 || rand(1) < dp) && real(A0) < real(B0) && imag(A0)<imag(B0)+0.5
            ...
            if first_detection == 0 && est_ttc == 1 % a loop that collects information at first moment of evasive action
                ...
                for k=1:NTTC
                    if k>length(row)%number of observed walks satisfying init. cond.
                        ttc = ttc_simulator_double_momentumV2(A0,B0, stepsize, theta, r, driver_prop, plot_setting);
                        stoch_ttc_FEA(i,k) = ttc;
                    else
                        ttc = ttc_simulator_history(A0,B0,stepsize,theta,r,driver_prop,historyA{1},row,col,k,plot_setting);
                        stoch_ttc_FEA(i,k) = ttc;
                        %pause(0.1)
                        %                     ttc = ttc_simulator_double_momentumV2(A0,B0, stepsize, theta, r, driver_prop, plot_setting);
                    end
...
% 
%                     title(sprintf("collision, %d", decision_state))
%                     hold on
...
%                 end
            ...
            plot_pos(A1, B1, pause_length, xinit, r, plotting,detection_status) 
