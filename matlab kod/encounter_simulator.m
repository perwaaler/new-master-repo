...
N_enc = 1000;                           % number of encounters
...
encData.states = cell(N_enc,1);
encData.type   = zeros(N_enc,1);
encData.decisions = cell(N_enc,1);
encData.length    = nan(150,1);
...
%             time_diff = temporal_sep(enc,RUprop,stat,plots);
            ...
            genInfo.nIter = 5;
            [S1,S2] = smoothenRateOfChange(enc,stat,3);
            Tsep = spline_pred_path(S1,S2,80,sparse_ind,genInfo);
            ...
            if S(1).EA==1 % if RU1 has engaged EA
                ...
                if stat.decision_count(1) == 1
                    actionAngleA = Tsep.actionAngle(1);
                end
                
%                 plots.col(1) = decision2color(decision(1));
                S(1) = take_step(S(1), S(2).speed,...
                                 S(1).theta + actionAngleA, max_delta(1));
            else
                % take normal step
            ...
            if S(2).EA==1 % if RU1 has engaged EA
                ...
                if stat.decision_count(2) == 1
                    actionAngleB = Tsep.actionAngle(2);
                end
                
                S(2) = take_step(S(2), S(2).speed,...
                S(2).theta + actionAngleB, max_delta(2));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    encData.time_diff{i} = enc.time_diff;
    encData.states{i}    = enc.states;
    encData.type(i)      = stat.type;
    encData.decisions{i} = enc.decisions;
    encData.length(i)    = stat.frame;
...
all_data{kk,12} = (sum(stat.type==-1)+sum(stat.type==-2))/N_enc;  